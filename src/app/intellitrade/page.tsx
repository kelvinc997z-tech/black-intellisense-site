        ...
        {view === 'terminal' ? (
          <main className="flex-1 grid lg:grid-cols-12 gap-12 overflow-hidden">
            ...
            <div className="lg:col-span-4 flex flex-col gap-8">
               <div className="flex-1 bg-[#050505] border border-white/10 p-10 flex flex-col rounded-[3rem] shadow-2xl relative">
                  <div className="flex bg-zinc-900/50 p-1.5 rounded-2xl mb-12">
                  ...
                  <form onSubmit={handleExecute} className="flex-1 flex flex-col justify-between">
                    <div className="space-y-10">
                       <div className="space-y-4">
                          <label className="text-[10px] text-zinc-600 uppercase tracking-widest ml-4">Execution Amount</label>
                          <div className="relative">
                             <input type="number" step="any" value={orderForm.amount} onChange={(e) => setOrderForm({...orderForm, amount: e.target.value})} placeholder="0.00" className="w-full bg-black border border-white/10 p-8 rounded-3xl text-4xl font-black text-white outline-none focus:border-blue-600 font-mono" />
                       ...
                       {orderForm.amount && (
                         <div className="bg-blue-600/5 border border-blue-500/20 p-8 rounded-[2rem] space-y-6 font-mono">
                            <div className="flex justify-between text-[10px] text-zinc-500 uppercase"><span>Locked Rate</span><span>Rp {currentPrice.toLocaleString('id-ID')}</span></div>
                            <div className="flex flex-col pt-4 border-t border-white/5"><span className="text-[10px] font-black text-blue-400 uppercase mb-2">Aggregate Value</span><span className="text-4xl font-black text-white italic font-heading">Rp {(parseFloat(orderForm.amount) * currentPrice).toLocaleString('id-ID')}</span></div>
                         </div>
                       )}
                    </div>
                    <button type="submit" disabled={isProcessing} className={`w-full py-10 rounded-[2.5rem] text-xl font-black tracking-widest uppercase transition-all flex items-center justify-center gap-6 ${isProcessing ? 'bg-zinc-900 text-zinc-700' : 'bg-white text-black hover:bg-blue-600 hover:text-white shadow-2xl'}`}>
          ...
          <main className="flex-1 bg-black border border-white/5 p-12 overflow-y-auto rounded-[3rem]">
             <div className="flex justify-between items-end mb-12 pb-8 border-b border-white/5">
                <div><h2 className="text-4xl font-black italic uppercase text-white font-heading">Audit Logs</h2><p className="text-[10px] tracking-widest text-zinc-600 uppercase mt-2">Institutional Ledger History</p></div>
                <button onClick={exportPDF} className="flex items-center gap-3 px-8 py-4 border border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all rounded-full"><FileDown size={18} /> Export PDF</button>
             </div>
             <div className="space-y-6 font-mono">
                {history.map((t, i) => (
                  <div key={i} className="grid grid-cols-6 p-8 border border-white/5 bg-zinc-900/10 hover:bg-zinc-900/20 transition-all items-center rounded-3xl">
                     <div className="flex flex-col"><span className="text-[8px] text-zinc-600 mb-1">Ticket</span><span className="text-sm font-bold text-blue-500">{t.id}</span></div>
                     <div className="flex flex-col"><span className="text-[8px] text-zinc-600 mb-1">Time</span><span className="text-sm font-bold">{t.date}</span></div>
                     <div className="flex flex-col"><span className="text-[8px] text-zinc-600 mb-1">Asset</span><span className="text-sm font-bold text-white">{t.asset}</span></div>
                     <div className="flex flex-col"><span className="text-[8px] text-zinc-600 mb-1">Op</span><span className={`text-sm font-bold uppercase ${t.side === 'buy' ? 'text-emerald-400' : 'text-red-400'}`}>{t.side}</span></div>
                     <div className="flex flex-col text-right"><span className="text-[8px] text-zinc-600 mb-1">Qty</span><span className="text-sm font-bold text-white">{t.amount}</span></div>
                     <div className="flex flex-col text-right"><a href={t.chain === 'BSC' ? `https://bscscan.com/tx/${t.fullHash}` : `https://polygonscan.com/tx/${t.fullHash}`} target="_blank" className="text-[10px] font-black text-blue-400 uppercase tracking-widest hover:text-white transition-all underline decoration-blue-500/20 underline-offset-4">Explorer</a></div>
                  </div>
                ))}
             </div>
          </main>
