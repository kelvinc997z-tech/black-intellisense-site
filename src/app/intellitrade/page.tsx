...
const IntelliTradeV6 = () => {
  ...
  const handleExecute = async (e: React.FormEvent) => {
    ...
    if (!orderForm.amount || isProcessing) return;
    
    setIsProcessing(true);
    const toastId = toast.loading("PREPARING BLOCKCHAIN PROTOCOL...");
    ...
    try {
      ...
      if (!chainConfig) {
        toast.error("PLEASE SWITCH TO BSC OR POLYGON IN METAMASK", { id: toastId });
      ...
      if (activeAsset.id === 'BNB' || activeAsset.id === 'POL') {
        ...
        toast.loading(`AWAITING ${activeAsset.id} CONFIRMATION...`, { id: toastId });
        ...
        toast.loading(`AWAITING ${activeAsset.id} TOKEN APPROVAL...`, { id: toastId });
      ...
      toast.success("TRANSACTION VERIFIED", { id: toastId, icon: <ShieldCheck className="text-emerald-500" /> });
      setOrderForm({ ...orderForm, amount: '' });
    } catch (err: any) {
      console.error("METAMASK_ERROR:", err);
      if (err.code === 'ACTION_REJECTED' || err.code === 4001) {
        toast.error("TRANSACTION_CANCELLED", { id: toastId });
      } else {
        toast.error("INSUFFICIENT_BALANCE_OR_GAS", { id: toastId });
      }
    } finally {
      setIsProcessing(false);
    }
  ...
  const exportPDF = () => {
    if (history.length === 0) return toast.error("No logs to export");
    const doc = new jsPDF();
    doc.setFillColor(0, 0, 0); doc.rect(0, 0, 210, 297, 'F');
    doc.setTextColor(255, 255, 255); doc.setFontSize(22); doc.text('BLACKINTELLISENSE PRO V.6', 14, 22);
    const tableData = history.map(t => [t.id, t.date, t.asset, t.side.toUpperCase(), t.amount, `Rp ${t.total}`, t.chain]);
    autoTable(doc, {
      startY: 45, head: [['Ticket ID', 'Timestamp', 'Asset', 'Side', 'Qty', 'Total IDR', 'Network']], body: tableData,
    ...
    <div className="min-h-screen bg-[#010102] text-zinc-300 font-sans selection:bg-blue-600/50 overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col p-4 md:p-6 lg:p-8 space-y-6 overflow-hidden">
        <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 border-b border-white/5 pb-10">
          <div className="flex items-center gap-6">
            <div className="relative group">
              ...
              <img src="/logo.jpg" alt="Logo" className="relative w-16 h-16 object-contain rounded-2xl" />
            </div>
            <div>
          ...
          <nav className="flex items-center gap-4 bg-zinc-900/50 p-1 rounded-sm border border-white/5">
            <button onClick={() => setView('terminal')} className={`px-4 py-2 flex items-center gap-2 text-[9px] font-black uppercase tracking-widest transition-all ${view === 'terminal' ? 'bg-blue-600 text-white' : 'text-zinc-500'}`}><LayoutGrid size={12} /> Terminal</button>
            <button onClick={() => setView('reports')} className={`px-4 py-2 flex items-center gap-2 text-[9px] font-black uppercase tracking-widest transition-all ${view === 'reports' ? 'bg-blue-600 text-white' : 'text-zinc-500'}`}><ListFilter size={12} /> Reports</button>
          </nav>

          <div className="flex items-center gap-6">
             <button onClick={connectWallet} className="px-6 py-2.5 bg-zinc-900 border border-white/10 text-[9px] font-black tracking-widest uppercase hover:bg-white hover:text-black transition-all">{account ? `[ AUTH_ID: ${account.slice(0,8)} ]` : "VERIFY_ENTITY"}</button>
          </div>
        </header>
        ...
        {view === 'terminal' ? (
          <main className="flex-1 grid grid-cols-12 gap-6 overflow-hidden">
            <div className="col-span-12 lg:col-span-8 flex flex-col gap-6 overflow-hidden">
              <div className="flex-1 bg-black border border-white/5 p-10 flex flex-col relative">
                <div className="flex justify-between items-start mb-12">
                   <div className="space-y-4">
                      <div className="flex items-center gap-3"><div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div><span className="text-[10px] font-black text-emerald-500 tracking-[0.4em]">MARKET_FEED_ACTIVE</span></div>
                      <div className="flex items-center gap-4 flex-wrap">
                        {ASSETS.map(asset => (
                          <button key={asset.id} onClick={() => setActiveAsset(asset)} className={`px-4 py-2 border rounded-sm text-[9px] font-black tracking-widest transition-all ${activeAsset.id === asset.id ? 'border-blue-500 bg-blue-500/10 text-blue-400' : 'border-white/5 text-zinc-600'}`}>{asset.id} / IDR</button>
                        ))}
                      </div>
                   </div>
                   <div className="text-right font-mono"><p className="text-[8px] text-zinc-600 tracking-widest uppercase mb-1">Vault Liquidity (PoR)</p><p className="text-sm font-black text-zinc-300 tracking-tighter">${vaultLiquidity} USDT</p></div>
                </div>
                <div className="flex-1 flex items-center"><div className="flex items-baseline gap-6"><h2 className="text-[14vw] lg:text-[12vw] font-black italic tracking-tighter text-white leading-[0.8] font-heading">{currentPrice.toLocaleString('id-ID')}</h2><span className="text-4xl font-black text-zinc-800 italic uppercase font-heading">IDR</span></div></div>
                <div className="grid grid-cols-4 gap-8 pt-8 border-t border-white/5">
                   {[{ l: "Spread", v: "0.04%", c: "text-emerald-400" }, { l: "Volatility", v: "High", c: "text-orange-400" }, { l: "Ping", v: "0.42ms", c: "text-blue-400" }, { l: "Router", v: "Smart-v6", c: "text-zinc-400" }].map((m, i) => (
                    <div key={i}><p className="text-[8px] text-zinc-600 tracking-widest uppercase mb-1">{m.l}</p><p className={`text-xs font-bold ${m.c}`}>{m.v}</p></div>
                   ))}
                </div>
              </div>
            </div>
            ...
            <div className="col-span-12 lg:col-span-4 flex flex-col gap-6">
               <div className="flex-1 bg-[#050505] border border-white/10 p-8 flex flex-col relative shadow-2xl">
                  ...
                  <form onSubmit={handleExecute} className="flex-1 flex flex-col">
                    <div className="space-y-6 flex-1">
                       ...
                       {orderForm.amount && (
                         <div className="bg-blue-600/5 border border-blue-500/20 p-6 space-y-4 font-mono">
                    ...
                    <button type="submit" disabled={!account || !orderForm.amount || isProcessing} className={`w-full py-8 text-xs font-black tracking-[0.8em] uppercase transition-all flex items-center justify-center gap-4 ${account && orderForm.amount && !isProcessing ? 'bg-white text-black hover:bg-blue-600 hover:text-white' : 'bg-zinc-900 text-zinc-700 cursor-not-allowed'}`}>
                       {isProcessing ? <RefreshCw className="animate-spin" /> : <>{orderForm.side === 'buy' ? 'Konfirmasi Beli' : 'Konfirmasi Jual'}</>}
                    </button>
                  </form>
               </div>
            </div>
          ...
          <main className="flex-1 bg-black border border-white/5 p-12 overflow-y-auto">
             <div className="flex justify-between items-end mb-10 pb-6 border-b border-white/5 font-sans"><div className="space-y-2"><h2 className="text-2xl font-black italic tracking-widest uppercase text-white font-heading">Audit Reports</h2></div><button onClick={exportPDF} className="flex items-center gap-2 px-6 py-3 border border-white/10 text-[9px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all font-mono"><FileDown size={14} /> Export</button></div>
             <div className="space-y-4 font-mono">
                {history.map((trade, i) => (
                  <div key={i} className="grid grid-cols-7 p-6 border border-white/5 bg-zinc-900/20 items-center">
                     <div className="flex flex-col"><span className="text-[7px] text-zinc-600 uppercase">Ticket</span><span className="text-xs font-bold text-blue-500">{trade.id}</span></div>
                     <div className="flex flex-col"><span className="text-[7px] text-zinc-600 uppercase">Time</span><span className="text-xs font-bold">{trade.date}</span></div>
                     <div className="flex flex-col"><span className="text-[7px] text-zinc-600 uppercase">Asset</span><span className="text-xs font-bold text-white">{trade.asset}</span></div>
                     <div className="flex flex-col"><span className="text-[7px] text-zinc-600 uppercase">Op</span><span className={`text-xs font-bold ${trade.side === 'buy' ? 'text-emerald-400' : 'text-red-400'}`}>{trade.side}</span></div>
                     <div className="flex flex-col text-right"><span className="text-[7px] text-zinc-600 uppercase">Qty</span><span className="text-xs font-bold">{trade.amount}</span></div>
                     <div className="flex flex-col text-right"><span className="text-[7px] text-zinc-600 uppercase">Total</span><span className="text-sm font-black text-white italic">Rp {trade.total}</span></div>
                     <div className="flex flex-col text-right"><a href={trade.chain === 'BSC-MAINNET' ? `https://bscscan.com/tx/${trade.fullHash}` : `https://polygonscan.com/tx/${trade.fullHash}`} target="_blank" className="text-[8px] font-black text-blue-400 uppercase tracking-widest hover:text-white transition-all">Explorer</a></div>
                  </div>
                ))}
             </div>
          </main>
        ...
        <footer className="h-10 border-t border-white/5 flex items-center justify-between text-[7px] font-bold text-zinc-800 uppercase tracking-[0.5em] font-mono">
           <div className="flex gap-10"><span>Protocol: v6.1.0-FIX</span><span>Handshake: Secure</span></div>
           <div className="flex items-center gap-4">{blocks.slice(0,2).map((b, i) => <span key={i} className="text-zinc-900 animate-pulse">BLOCK: {b}</span>)}</div>
        </footer>
      </div>
      <style jsx global>{`
