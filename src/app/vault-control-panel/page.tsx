      ...
      <div className="min-h-screen bg-[#020203] flex items-center justify-center p-8 font-sans">
        <div className="max-w-md w-full bg-zinc-900/40 border border-white/5 p-12 rounded-[3rem] text-center space-y-10 backdrop-blur-xl shadow-2xl">
          <div className="relative inline-block">
             ...
             <ShieldCheck className="relative text-blue-500 mx-auto" size={80} />
          </div>
    ...
    return (
      <div className="min-h-screen bg-[#020203] flex items-center justify-center p-8 font-sans">
        <div className="max-w-md w-full bg-red-500/5 border border-red-500/20 p-12 rounded-[3rem] text-center space-y-8 backdrop-blur-xl">
          ...
          <p className="text-[9px] text-zinc-600 uppercase tracking-widest leading-relaxed">Access to Vault Terminal restricted to Authoritative Signers only.</p>
      ...
      <div className="min-h-screen bg-[#020203] flex items-center justify-center p-8 font-sans">
        <div className="max-w-md w-full bg-zinc-900/40 border border-red-500/20 p-12 rounded-[3rem] text-center space-y-10 backdrop-blur-xl">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[9px] text-zinc-500 uppercase tracking-[0.2em]">Input Secondary Verification Key</p>
          </div>
          <form onSubmit={handlePinSubmit} className="space-y-8">
            <input 
              ...
              placeholder="••••••"
              className="w-full bg-black/50 border border-white/5 p-8 rounded-[2rem] text-center text-4xl font-black tracking-[0.5em] text-red-500 focus:border-red-600 focus:bg-black transition-all outline-none"
              autoFocus
            />
            <button 
              type="submit"
              className="w-full py-5 bg-red-600 text-white text-[11px] font-black uppercase tracking-widest rounded-2xl hover:bg-white hover:text-black transition-all shadow-lg shadow-red-600/20"
            >
              Verify Authority
            </button>
          </form>
      ...
      <div className="relative z-10 p-4 md:p-8 lg:p-12 max-w-7xl mx-auto space-y-12">
        <header className="flex flex-col md:flex-row justify-between items-center border-b border-white/5 pb-12 gap-8">
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 bg-red-600 rounded-[2rem] flex items-center justify-center shadow-[0_0_50px_rgba(220,38,38,0.4)] rotate-3">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:block text-right">
               <p className="text-[8px] text-zinc-600 uppercase mb-1">Authorization Node</p>
               <p className="text-[10px] font-black text-zinc-400 font-mono">{account}</p>
            </div>
            <Link href="/intellitrade" className="px-8 py-4 bg-zinc-900/50 border border-white/10 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all rounded-2xl flex items-center gap-3">
        ...
        <main className="space-y-12">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-zinc-900/30 border border-white/5 p-10 rounded-[3rem] backdrop-blur-md">
                 ...
                 <p className="text-[9px] text-zinc-600 uppercase tracking-widest mb-2 font-bold">Queue Depth</p>
                 <p className="text-5xl font-black text-white italic">{pendingOrders.length}</p>
              </div>
              <div className="bg-zinc-900/30 border border-white/5 p-10 rounded-[3rem] backdrop-blur-md">
                 <History className="text-emerald-500 mb-6" size={32} />
                 <p className="text-[9px] text-zinc-600 uppercase tracking-widest mb-2 font-bold">Processed</p>
              ...
              <div className="bg-zinc-900/30 border border-white/5 p-10 rounded-[3rem] backdrop-blur-md border-emerald-500/20">
                 ...
                 <p className="text-[9px] text-zinc-600 uppercase tracking-widest mb-2 font-bold">Operational</p>
                 <p className="text-xl font-black text-emerald-400 uppercase italic">Active Nodes Online</p>
              </div>
           </div>
           ...
           <section className="bg-zinc-900/20 border border-white/5 rounded-[3.5rem] overflow-hidden backdrop-blur-xl">
              <nav className="flex border-b border-white/5 p-4 gap-4 bg-black/20">
                 <button 
                  ...
                  className={`px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'queue' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}
                 >
                    Active Queue
                 </button>
                 <button 
                  onClick={() => setActiveTab('history')}
                  className={`px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-white'}`}
                 >
                    Audit History
                 </button>
              </nav>
                   ...
                   <div className="space-y-6">
                      {pendingOrders.length === 0 ? (
                        <div className="py-32 text-center space-y-4">
                           ...
                           <p className="text-zinc-700 uppercase text-[10px] font-black tracking-[0.5em]">No pending transactions detected in network</p>
                        </div>
                      ) : (
                        pendingOrders.map((p, i) => (
                          <div key={i} className="grid grid-cols-1 md:grid-cols-12 p-10 border border-white/5 bg-black/40 items-center rounded-[2.5rem] gap-8 hover:border-red-500/30 transition-all group">
                             <div className="md:col-span-2 space-y-1">
                                <span className="text-[8px] text-zinc-600 uppercase font-black block">Batch ID</span>
                                <span className="text-xs font-bold text-red-500 font-mono uppercase">{p.id.slice(0,8)}</span>
                             </div>
                             <div className="md:col-span-3 space-y-1">
                                <span className="text-[8px] text-zinc-600 uppercase font-black block">Recipient Wallet</span>
                                <span className="text-[10px] font-bold text-zinc-400 truncate block font-mono">{p.targetAddress}</span>
                             </div>
                             <div className="md:col-span-2 space-y-1 text-center md:text-left">
                                <span className="text-[8px] text-zinc-600 uppercase font-black block">Resource / Qty</span>
                                <span className="text-sm font-black text-white uppercase italic">{p.amount} {p.asset}</span>
                             </div>
                             <div className="md:col-span-2 space-y-1 text-center md:text-left">
                                <span className="text-[8px] text-zinc-600 uppercase font-black block">Payment Hash</span>
                                <a href={`${CHAINS[p.chainId]?.explorer}${p.paymentHash}`} target="_blank" className="text-[10px] font-bold text-blue-500 truncate block hover:text-white underline decoration-blue-500/30 font-mono">View Payment</a>
                             </div>
                             <div className="md:col-span-3">
                                <button 
                                  onClick={() => approveOrder(p)}
                                  disabled={isProcessing}
                                  className="w-full py-5 bg-red-600 text-white text-[11px] font-black uppercase rounded-2xl hover:bg-white hover:text-black transition-all shadow-xl shadow-red-600/10 flex items-center justify-center gap-3 disabled:opacity-50 group-hover:scale-[1.02]"
                                >
                                  {isProcessing ? <RefreshCw className="animate-spin" size={16} /> : <><ArrowUpRight size={16} /> Dispatch Asset</>}
                                </button>
                             </div>
                          </div>
                        ))
                        ...
                        <div key={i} className="flex justify-between items-center p-6 border border-white/5 bg-black/20 rounded-2xl">
                           <div className="flex gap-10 items-center">
                              <div className="flex flex-col">
                                 <span className="text-[8px] text-zinc-600 uppercase">Ticket</span>
                                 <span className="text-xs font-bold text-zinc-400">{t.id.slice(0,8)}</span>
                              </div>
                              <div className="flex flex-col">
                                 <span className="text-[8px] text-zinc-600 uppercase">Operation</span>
                                 <span className={`text-[10px] font-black uppercase ${t.side === 'buy' ? 'text-blue-500' : 'text-emerald-500'}`}>{t.side}</span>
                              </div>
                              <div className="flex flex-col">
                                 <span className="text-[8px] text-zinc-600 uppercase">Value</span>
                                 <span className="text-xs font-black text-white">{t.amount} {t.asset}</span>
                              </div>
                           </div>
                           <a href={`${CHAINS[t.chainId]?.explorer}${t.txHash}`} target="_blank" className="p-3 bg-zinc-900 rounded-xl hover:bg-white hover:text-black transition-all"><ExternalLink size={14} /></a>
                        </div>
                      ))}
                   </div>
                 )}
        ...
        <footer className="h-10 border-t border-white/5 flex items-center justify-between text-[8px] font-bold text-zinc-800 uppercase tracking-[0.5em] font-mono">
           <span>Authorization: CLEARANCE_LEVEL_0</span>
           <span>Bridge Status: LINKED</span>
        </footer>
      </div>
    </div>
  );
